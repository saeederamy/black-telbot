...
BTN_UL = "Upload to Drive"
BTN_SETUP = "Setup Credentials"
BTN_ABOUT = "About & Donate"
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in [BTN_DL, BTN_UL, BTN_SETUP, BTN_ABOUT]:
        ...
        if text == BTN_ABOUT:
            ...
            if not os.path.exists(os.path.join(CREDS_DIR, f"{uid}.json")):
                await update.message.reply_text("Please use 'Setup Credentials' first to connect your Google Drive.")
            else:
    ...
    if state == 'WAIT_JSON' and update.message.document:
        ...
        kb = [
            ...
            [InlineKeyboardButton("720p / Med", callback_data='q_720')],
        ...
        await msg.edit_text("Uploading to your Google Drive...")
        ...
        try:
            ...
            await msg.edit_text("Upload Failed. Check if the folder is shared correctly with the bot email.")
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('q_'):
        ...
        await query.edit_message_text(f"Downloading media ({quality})... Please wait.")
        
        format_sel = 'bestvideo+bestaudio/best'
        if quality == '1080': 
            format_sel = 'bestvideo[height<=1080]+bestaudio/bestvideo[width<=1080]+bestaudio/best'
        elif quality == '720': 
            format_sel = 'bestvideo[height<=720]+bestaudio/bestvideo[width<=720]+bestaudio/best'
        elif quality == 'mp3': 
